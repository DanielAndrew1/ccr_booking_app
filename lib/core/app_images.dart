abstract class AppIcons {
  static String add = "assets/svg/add-square.svg";
  static String bg = "assets/svg/bg-decoration.svg";
  static String booking = "assets/svg/booking.svg";
  static String inventory = "assets/svg/box.svg";
  static String calendarAdd = "assets/svg/calendar-add.svg";
  static String calendar = "assets/svg/calendar.svg";
  static String phone = "assets/svg/call.svg";
  static String close = "assets/svg/close-circle.svg";
  static String save = "assets/svg/folder-2.svg";
  static String globe = "assets/svg/globe.svg";
  static String home = "assets/svg/home-2.svg";
  static String info = "assets/svg/info-circle.svg";
  static String login = "assets/svg/login.svg";
  static String logout = "assets/svg/logout.svg";
  static String medal = "assets/svg/medal.svg";
  static String edit = "assets/svg/message-edit.svg";
  static String notification = "assets/svg/notification-bing.svg";
  static String client = "assets/svg/profile-2user.svg";
  static String search = "assets/svg/search-normal.svg";
  static String pickUp = "assets/svg/send-square.svg";
  static String email = "assets/svg/sms.svg";
  static String tick = "assets/svg/tick-circle.svg";
  static String trash = "assets/svg/trash.svg";
  static String userAdd = "assets/svg/user-add.svg";
  static String userSearch = "assets/svg/user-search.svg";
  static String profile = "assets/svg/user.svg";
  static String verify = "assets/svg/verify.svg";
  static String returns = "assets/svg/vuesax.svg";
  static String wallet = "assets/svg/wallet.svg";
}

abstract class AppImages {
  static String appIcon = "assets/png/App_Icon.png";
  static String darkAppIcon = "assets/png/Dark_App_Icon.png";
  static String icon = "assets/png/icon.png";
  static String logo = "assets/png/logo.png";
}
